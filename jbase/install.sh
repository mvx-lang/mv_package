...
# THE CLIENT GETS ITS OWN LIBRARY DIRECTORY, the way a package does.
#
# Left alone jBASE catalogs into $HOME/lib/lib0.so -- shared with everything
# else the user catalogs, and named after nothing.  When it failed to load, the
# report was "/home/rocky/lib/lib0.so.84: undefined symbol: JBCURLPOST", naming
# neither the package at fault nor the caller (#128), and repairing it meant
# clearing a library that is not ours to clear.
#
# Its own directory fixes both: the library can be named libmvpkg%n.so, and
# mvpkg-jbrebuild replaces only what mvpkg put there.  $HOME/lib is left alone
# and stays on the search path AFTER this one, so a client cataloged there by an
# older install is shadowed rather than deleted.
CLIENTLIB="$MVPKG_STORE/mvpkg-lib"
mkdir -p "$CLIENTLIB"
# libname takes effect only on an EMPTY library directory -- CATALOG reads the
# existing lib.el otherwise and keeps the old name -- so the definition goes in
# before the first catalog, and a re-install clears what it wrote last time.
rm -f "$CLIENTLIB"/libmvpkg*.so* "$CLIENTLIB"/lib0.so* "$CLIENTLIB"/lib.el
cat > "$CLIENTLIB/jLibDefinition" <<'JLIBEOF'
## GENERATED BY MVPKG -- the client's shared object is named for the client.
## Changing libname here without clearing the old library leaves both on
## JBCOBJECTLIST, resolving the same name two ways.
libname = libmvpkg%n.so
exportname = lib.el
objectdir = obj
maxsize = 1048576
JLIBEOF
export JBCDEV_LIB="$CLIENTLIB" JBCDEV_BIN="$CLIENTLIB"
say "the client's library: $CLIENTLIB/libmvpkg0.so"
...
if [ -f "$HERE/mvpkg-jblib" ]; then
   ...
   # Write the managed line now, so the client's own library is on the search
   # path before any package has been installed to put it there.
   "$MVPKG_STORE/mvpkg-jblib" add "$CLIENTLIB" >/dev/null 2>&1 || true
fi
...
if [ -f "$HERE/mvpkg-jbrebuild" ]; then
   ...
   say "      if the client library ever fails to load (\"undefined symbol\"), run"
   say "      it from this account to rebuild: $MVPKG_STORE/mvpkg-jbrebuild"
   ...
   say "      Packages go FIRST, then this client's library, then \$HOME/lib."
   say "      jBASE searches in order, and the client carries cut-down bootstrap"
   say "      copies of HTTPGET/HTTPGETFILE/JSONDECODE/MAPFIELD -- they exist only"
   say "      so MVPKG can fetch the packages providing those same names, so a real"
   say "      package has to win. \$HOME/lib stays last: it is jBASE's default and"
   say "      shadows any client an older install cataloged there."
