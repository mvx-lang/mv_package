...
# with it.  The client neither knows nor cares which.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
ACCT="${1:?usage: mkrelease.sh <account-dir> <name> <version> [description]}"
...
echo "registered $NAME $VER -> registry/$NAME/$TAR"
